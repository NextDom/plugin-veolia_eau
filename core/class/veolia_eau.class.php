...
class veolia_eau extends eqLogic
{
    ...
    private $urlToutSurMonEau = 'https://www.toutsurmoneau.fr';
    private $urlEauServices = 'https://www.eau-services.com';
    private $urlAgenceEauDuGrandLyon = 'https://agence.eaudugrandlyon.com';
    private $urlServiceClientVeoliaEau = 'https://www.service-client.veoliaeau.fr';
    ...
    public function postUpdate()
    {
        ...
        if (!is_object($cmdlogic)) {
            $veoliaEauCmd = (new veolia_eauCmd())
                    ->setName(__('Index', __FILE__))
                    ->setEqLogic_id($this->id)
                    ->setLogicalId('index')
                    ->setConfiguration('data', 'index')
                    ->setType('info')
                    ->setSubType('numeric')
                    ->setUnite('L')
                    ->setIsHistorized(1);
        ...
        if (!is_object($cmdlogic)) {
            $veoliaEauCmd = (new veolia_eauCmd())
                    ->setName(__('Consommation', __FILE__))
                    ->setEqLogic_id($this->id)
                    ->setLogicalId('conso')
                    ->setConfiguration('data', 'conso')
                    ->setType('info')
                    ->setSubType('numeric')
                    ->setUnite('L')
                    ->setIsHistorized(1);
        ...
        if (!is_object($cmdlogic)) {
            $veoliaEauCmd = (new veolia_eauCmd())
                    ->setName(__('Mesuré / Estimé', __FILE__))
                    ->setEqLogic_id($this->id)
                    ->setLogicalId('typeReleve')
                    ->setConfiguration('data', 'typeReleve')
                    ->setType('info')
                    ->setSubType('string')
                    ->setIsHistorized(0);
            $veoliaEauCmd->save();
        ...
        if (!is_object($cmdlogic)) {
            $veoliaEauCmd = (new veolia_eauCmd())
                    ->setName(__('Date', __FILE__))
                    ->setEqLogic_id($this->id)
                    ->setLogicalId('dateReleve')
                    ->setConfiguration('data', 'dateReleve')
                    ->setType('info')
                    ->setSubType('string')
                    ->setIsHistorized(0);
            $veoliaEauCmd->save();
    ...
    public function getConso()
    {
        ...
        switch (intval($this->getConfiguration('website'))) {
            case 2:
                $url_login        = $this->urlEauServices . '/default.aspx';
                ...
                $url_consommation = $this->urlEauServices . '/mon-espace-suivi-personnalise.aspx?mm=' . $month . '&d=';
                $url_releve_csv   = $this->urlEauServices . '/mon-espace-suivi-personnalise.aspx?ex=' . $month . '&mm=' . $month . '&d=';
            ...
            case 3:
                $url_login        = $this->urlAgenceEauDuGrandLyon . '/default.aspx';
                $url_consommation = $this->urlAgenceEauDuGrandLyon . '/mon-espace-suivi-personnalise.aspx';
                $url_releve_csv   = $this->urlAgenceEauDuGrandLyon . '/mon-espace-suivi-personnalise.aspx?ex=9/2016&mm=9/2016&d=';
                $datas            = array(
                    'login=' . urlencode($this->getConfiguration('login')),
                    'pass=' . urlencode($this->getConfiguration('password')),
                    'connect=OK',
            ...
            case 4:
                $url_token          = $this->urlToutSurMonEau . '/mon-compte-en-ligne/je-me-connecte';
                $tokenFieldName     = '_csrf_token';
                $url_login          = $this->urlToutSurMonEau . '/mon-compte-en-ligne/je-me-connecte';
                $url_consommation   = $this->urlToutSurMonEau . '/mon-compte-en-ligne/historique-de-consommation';
            ...
            case 1:
            default:
                $url_token        = $this->urlServiceClientVeoliaEau . '/connexion-espace-client.html';
                $tokenFieldName   = 'token';
                $url_login        = $this->urlServiceClientVeoliaEau . '/home.loginAction.do';
                $url_consommation = $this->urlServiceClientVeoliaEau . '/home/espace-client/votre-consommation.html?vueConso=releves';
                $url_releve_csv   = $this->urlServiceClientVeoliaEau . '/home/espace-client/votre-consommation.exportConsommationData.do?vueConso=releves';
                $datas            = array(
